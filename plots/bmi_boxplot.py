...
PLOT_P_VALUES = True
signif = {
	'CC': [ (0, 2, None), (0, 1, None), (0, 3, None), (1, 2, None), (1, 3, None), (2, 3, None) ],
	'NC': [ (0, 2, None), (0, 1, None), (0, 3, None), (1, 2, 0.231),  (1, 3, None), (2, 3, None) ],
}
...
def plot_boxplots(query, cohort) :
	...
	check_dataframe_columns(df, ['cluster', 'bmi_at_baseline'])
	...
	for index, cluster in enumerate(cluster_ordering) :
		...
		bp = ax.boxplot(subset['bmi_at_baseline'], positions=[index],
	...
	if PLOT_P_VALUES :
		...
		for idx, cluster in enumerate(cluster_ordering) :
			subset = df[df['cluster'] == cluster]['bmi_at_baseline']
			q3 = subset.quantile(0.75)
			if q3 > y_start :
				y_start = q3
		
		tick_h_distance = 0.05
		h = (df['bmi_at_baseline'].max() - df['bmi_at_baseline'].min()) * 0.05
	...
	# Plot horizontal lines at integer BMI values
	for bmi in range(ceil(y_lower_limit/10)*10, ceil(y_upper_limit/10)*10, 10) :
		plt.axhline(y=bmi, color='gray', linestyle='-', linewidth=0.5, alpha=0.7)
	
	# Set plot title and labels
	plt.title(f'BMI distribution by cluster ({cohort})')
	...
	svg_filepath = get_plot_filepath(f'bmi_boxplots_{cohort}.svg')
...
def plot_bmi_boxplots() :
	for cohort in ['CC', 'NC'] :
		...
		query = f'''
			SELECT
				cluster,
				e.valore::float AS bmi_at_baseline
			FROM dati2.esamilaboratorioparametricalcolati as e
			JOIN (
				SELECT idcentro, idana, cluster, MIN(e2.data) AS data
				FROM dati2.esamilaboratorioparametricalcolati AS e2
				JOIN { cluster_table_name } AS cl
				USING (idcentro, idana)
				WHERE e2.codicestitch = 'STITCH001'                 --bmi
					AND date_part('year', e2.data) >= cl.annoinizio --baseline
					AND e2.valore::float BETWEEN 16 AND 60
				GROUP BY idcentro, idana, cluster
				) first_valid_bmi_exam
			USING (idcentro, idana, data)
			WHERE e.codicestitch = 'STITCH001'; --bmi'''
