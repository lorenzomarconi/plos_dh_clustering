...
signif = {
	...
	'NC': [ (0, 2, None), (0, 1, None), (0, 3, None), (1, 2, None),  (1, 3, None), (2, 3, None) ],
}
...
def plot_boxplots(query, cohort) :
	...
	check_dataframe_columns(df, ['cluster', 'num_prescriptions'])
	...
	for index, cluster in enumerate(cluster_ordering) :
		...
		bp = ax.boxplot(subset['num_prescriptions'], positions=[index],
	...
	if PLOT_P_VALUES :
		...
		for idx, cluster in enumerate(cluster_ordering) :
			subset = df[df['cluster'] == cluster]['num_prescriptions']
			q3 = subset.quantile(0.75)
			if q3 > y_start :
				y_start = q3
		
		tick_h_distance = 0.05
		h = (df['num_prescriptions'].max() - df['num_prescriptions'].min()) * 0.05
	...
	plt.title(f'Number of prescriptions by cluster ({cohort})')
	...
	svg_filepath = get_plot_filepath(f'num_prescriptions_boxplots_{cohort}.svg')
	...
	for cohort in ['CC', 'NC'] :
		...
		query = f'''
			...
			SELECT
				cluster,
				CASE
					WHEN (idcentro, idana) NOT IN (SELECT idcentro, idana FROM visits_with_prescription)
					THEN 0 ELSE COUNT(*)::DECIMAL
				END AS num_prescriptions
			FROM visits_with_prescription
			...
			ORDER BY cluster, num_prescriptions'''
